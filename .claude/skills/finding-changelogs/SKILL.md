...
Add the source and immediately set it to paused priority. CLI: `releases admin source create <name> --url <url> --org <org> --type github` then `releases admin source update <identifier> --priority paused`. Typed tools: `manage_source` action "add" then `manage_source` action "edit" with identifier and fetch_priority: "paused".
