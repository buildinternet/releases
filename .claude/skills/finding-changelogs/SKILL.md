...
Evaluate a URL to determine the best ingestion method: `releases admin discovery evaluate <url> --json`.
...
After adding sources, mark the primary one: `releases admin source update <identifier> --primary`. Only one source per org should be primary. If there's no clear top-level changelog, don't mark any as primary.
...
| Blume                                    | `/changelog/rss.xml`                                 | Yes (`.md`)     | Yes    | Self-hosted Astro generator. Detected via the in-head `blume-theme` marker. RSS items are title-only (no body) → auto-enriched by following each entry link.  |
...
Add the source and immediately set it to paused priority: `releases admin source create <name> --url <url> --org <org> --type github` then `releases admin source update <identifier> --priority paused`.
...
Use product and org management operations to organize what you find: `releases admin product create`, `releases admin org tag add`. `releases categories` lists the valid category values.
