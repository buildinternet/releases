...
# Logic lives here so apps/web/vercel.json's ignoreCommand stays under the 256-char schema limit.
set -u
...
# Repo-root :/ pathspecs stay correct regardless of Vercel Root Directory (usually apps/web/).
if git diff --quiet "${VERCEL_GIT_PREVIOUS_SHA:-HEAD^}" HEAD -- \
  :/apps/web :/packages :/scripts :/bun.lock :/package.json; then
