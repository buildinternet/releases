import { formatStars } from "@/lib/format-stars";

interface RepoStarsProps {
  stars?: number | null;
  className?: string;
}

/**
 * Compact GitHub star count with a small star glyph. Renders nothing when the
 * count is null/undefined (never fetched). Inline octicon SVG — house style
 * avoids emoji and external icon libraries.
 */
export function RepoStars({ stars, className }: RepoStarsProps) {
  if (stars == null) return null;
  return (
    <span
      className={`inline-flex items-center gap-1 text-stone-400 dark:text-stone-500 ${className ?? ""}`}
      title={`${stars.toLocaleString()} stars on GitHub`}
    >
      <svg viewBox="0 0 16 16" width="11" height="11" aria-hidden="true" fill="currentColor">
        <path d="M8 .25a.75.75 0 0 1 .673.418l1.882 3.815 4.21.612a.75.75 0 0 1 .416 1.279l-3.046 2.97.719 4.192a.75.75 0 0 1-1.088.791L8 12.347l-3.766 1.98a.75.75 0 0 1-1.088-.79l.72-4.194L.818 6.374a.75.75 0 0 1 .416-1.28l4.21-.611L7.327.668A.75.75 0 0 1 8 .25Z" />
      </svg>
      <span className="tabular-nums">{formatStars(stars)}</span>
    </span>
  );
}
