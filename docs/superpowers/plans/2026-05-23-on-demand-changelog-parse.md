...
- **Not changed:** `workers/api/src/route-namespaces.ts` (`"changelog"` already in `publicReadRoutes`).
- **Coverage-gate correction:** `scripts/check-openapi-coverage.ts` DID need an `ALLOWLIST` entry for `POST /changelog/parse`. The plan originally assumed `hideInProduction` kept it outside the gate, but the gate compares routes _registered_ under a public-read namespace against the prod spec — a hidden route is registered but absent from the spec, so it must be allowlisted (exactly as `/changelog/fetch` is). Run `bun scripts/check-openapi-coverage.ts` as part of verification.
