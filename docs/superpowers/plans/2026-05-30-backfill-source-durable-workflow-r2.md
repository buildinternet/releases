...
- [ ] **Step 2:** Run → FAIL.
- [ ] **Step 3:** Implement: after body-`via` is known, if `via === "firecrawl" && (await flag(env.FLAGS, env.BACKFILL_WORKFLOW_ENABLED, FLAGS.backfillWorkflow)) && env.BACKFILL_SOURCE_WORKFLOW`, `create({ id: \`backfill-${sourceId}-${scheduledTime}\`, params })`and return`202 { instanceId, statusUrl, async: true }`. Else fall through to the existing synchronous `runSourceBackfill`. Add the status GET mirroring `batch-summarize/status`(reuse`WORKFLOW_NOT_FOUND_RE`). Note: `scheduledTime` must come from a param/`Date.now()` at request time (not inside a workflow step).
