...
- **SERVER_ONLY admin endpoints exist** and are callable only via `auth.api.*`
  ...
  the secret, maps `snake_case` input, JSON-encodes `redirect_uris`, and returns
  the `reloc_`-prefixed secret once.
...
| -------------------- | ------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Create**           | `POST /v1/admin/oauth/clients`                         | `auth.api.adminCreateOAuthClient` (`SERVER_ONLY`). Passes through `redirect_uris`, `scope`, `client_name`, `client_uri`, `logo_uri`, `type`, `token_endpoint_auth_method`, `grant_types`, `require_pkce`, and `skip_consent` (from `--trusted`). Returns the `reloc_`-prefixed secret **once** (or none for a public client). |
