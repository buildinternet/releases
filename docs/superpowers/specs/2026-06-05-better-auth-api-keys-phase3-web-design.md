...
2. **The plugin's `list`/`delete` are already user-scoped** — `listApiKeys` returns only the session user's keys; `deleteApiKey` requires the session cookie and enforces ownership. We still wrap them (for one consistent surface and response shape), but they need no extra ownership logic of our own.
...
| List / delete               | **Wrapped** `GET` / `DELETE`, but rely on the plugin's built-in user-scoping/ownership        | One consistent surface + response shape; no need to reinvent ownership (plugin scopes to the session user).                                                                                                                                      |
...
- **`GET /v1/api-keys`** — list. `auth.api.listApiKeys({ query: {}, headers })` (plugin scopes to the session user). Project each row to `{ id, name, start, scope, enabled, remaining, lastRequest, createdAt, expiresAt }`. `key` is never present (the plugin omits it) and never added.
- **`DELETE /v1/api-keys/:id`** — revoke (hard delete). `auth.api.deleteApiKey({ body: { keyId: id }, headers })`; the plugin enforces ownership against the session. A key that isn't found or isn't the caller's → `404 { error: "not_found" }` (do not distinguish "exists but not yours" from "absent" — no cross-user existence oracle).
...
- Apply the existing per-IP `publicRateLimitMiddleware` to the namespace to blunt create-spam (a session can still hold many keys; this caps request _rate_, not key count).
...
2. **Ownership** — list/delete derive the owner from the session (never a client-supplied userId); create passes `userId: session.user.id` (the server-only field), so a user can only mint/list/revoke their own keys. Delete of a non-owned/absent key returns an indistinct `404`.
