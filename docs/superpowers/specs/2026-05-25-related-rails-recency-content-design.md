...
## Follow-up — global-rail quality floor (post-deploy smoke, 2026-05-25)

A second prod smoke showed the content fix working well on content-rich anchors (recent, on-topic global neighbors) but degrading for a **thin anchor** like `v2.1.150`: once the recent content-free neighbors are excluded, only stale content-rich ones remain (2023–2024), so the global rail surfaced older items than the original complaint. Recency decay ranks them correctly but can't conjure recent+rich+similar neighbors that don't exist for a content-free anchor.

Added `RELATED_GLOBAL_MIN_RANK = 0.2`: a minimum combined rank (`cosine × recency × contentWeight`) applied **only** when `scope === "global"`. Candidates below it are dropped before slicing, so a thin anchor's weak global rail collapses (the web hides an empty rail) rather than rendering stale filler. Calibrated from the smoke: strong rails top out ~0.45–0.7, the stale `v2.1.150` rail topped ~0.14. The org rail keeps `minRank = 0` so a slow-moving org's older releases still fill "More from {org}". This is the soft, data-driven version of the "hard recency floor on global" option weighed during brainstorming.
