...
3. **No live `relk_` tokens exist yet** (system merged 2026-05-20, bootstrap not done, only the internal `mint-token.ts`), so re-prefixing the machine lane to `relm_` and re-issuing is free.
...
- Preserve the `relk_` prefix (GitHub secret-scanning candidate) for the public-facing user keys.
...
- **Migrating machine tokens into Better Auth** — `internal`/`agent` tokens stay on the `relm_` lane (decided: a thin machine lane, not synthetic service-user accounts).
...
| Machine tokens       | Keep on the existing `relk_` system, re-prefixed **`relm_`**      | Least disruption; preserves the fast cross-worker pure verifier for machines; keeps the metered human path and the unmetered machine path cleanly separated.                                                        |
| Lane routing         | By **prefix** (`relk_` user, `relm_` machine, exact root key)     | Unambiguous; each verifier returns an authoritative verdict — no fall-through guessing across verifiers, no double-metering risk.                                                                                   |
...
| Key prefix           | `relk_` for user keys (public face)                               | Most likely to leak into client code / `.env`; best candidate for GitHub secret-scanning registration. Set via the plugin's `defaultPrefix`.                                                                        |
...
| ---------------- | -------------------- | ------------------------------ | --------------------------- | -------- | ------------------------------ |
| **User keys**    | `relk_`              | a Better Auth `user`           | Better Auth `apikey` table  | yes      | `auth.api.verifyApiKey`        |
| **Machine keys** | `relm_`              | `internal` / `agent` principal | existing `api_tokens` table | no       | existing pure `verifyApiToken` |
| **Static root**  | (none — exact match) | implicit root                  | —                           | no       | constant-time string compare   |

Routing is by prefix, so a token is eligible for exactly one verifier and each verdict is authoritative. Because there are no live tokens, the machine lane's mint prefix changes from `relk_` to `relm_` with no migration of existing credentials. If GitHub secret-scanning partner registration is pursued, both `relk_` and `relm_` patterns are registered.

**Key format note:** user keys adopt Better Auth's `<prefix><opaque-secret>` shape (the `relk_` prefix preserved via the plugin's `defaultPrefix`; lookup is by the stored hash of the whole key). They do **not** keep the legacy `relk_<lookupId>_<secret>` tri-part structure — that structure stays only on the `relm_` machine lane. Any tooling that parses the tri-part form must key off the `relm_` prefix.
...
A small pure shim lives next to the existing scope helpers:

- `scopeToPermissions(scope)` — expand a ladder scope to its cumulative permission set (creation side).
- `permissionsToScope(perms)` — collapse a verified key's permissions back to the highest `read|write|admin` label (verify side).
- `requiredPermissionFor(route/method)` — the permission a given route demands, fed to `verifyApiKey({ permissions })`.

After a successful Better Auth verify, the middleware calls `permissionsToScope` and sets the existing `scopes` array on the Hono context **exactly as today**. Every existing scope-based guard (`publicReadAuthMiddleware`, `authMiddleware`, the admin `isValidBearerAuth` field-unlock predicate) therefore keeps working with no change beyond the new verify branch.
...
2. starts with `relk_` → `auth.api.verifyApiKey({ key, permissions: requiredPermissionFor(route) })`. On `valid`, attach the user principal and set `scopes = permissionsToScope(key.permissions)`. **This is the single metering verify** (rate-limit counter + `remaining` decrement, deferred via `waitUntil`). Gated by `API_TOKENS_DISABLED` and `user-api-keys-enabled`.
3. starts with `relm_` → existing `verifyApiToken` against `api_tokens` _(unchanged logic; renamed prefix; unmetered)_.
4. else → reject (`401`).
...
- **`relm_` machine key** → local `verifyApiToken` via the shared pure verifier _(unchanged; unmetered)_.
- **`relk_` user key, MCP-native tool** (a tool that resolves entirely inside MCP and never calls the API worker — e.g. the AI tools `summarize_changes` / `compare_products`) → MCP calls a new `verifyApiKey` endpoint over the `API` binding → **metered once here**.
- **`relk_` user key, forwarding tool** (a tool that already proxies to the API worker — e.g. `maybeLookup` → `/v1/lookups`) → MCP **forwards the user key unchanged**; the API-worker middleware (§3) verifies + meters **once at the operation**. MCP does _not_ pre-verify these, so there is no double count — it propagates the API worker's `200`/`403`/`429` verbatim.
...
flowchart TD
    ...
    P -->|relk_| BAV["BA verifyApiKey<br/>METER ONCE"]
    P -->|relm_| LEG["legacy verifyApiToken<br/>no meter"]
    BAV --> SC["permissions → scopes"]
    LEG --> SC
    ROOT --> SC --> RT[route guard]

    W -->|MCP worker| MT{tool type}
    MT -->|"native tool, relk_"| BIND["API binding → verifyApiKey<br/>METER ONCE"]
    MT -->|"forwarding tool, relk_"| FWD[forward user key → API worker]
    MT -->|"relm_ / root"| MLOC["local verify<br/>no meter"]
    FWD --> AM
```
...
- **`api_tokens`** is unchanged except the machine lane's **generate and parse** prefix, which moves from `relk_` to `relm_` (the core generator + `parseApiToken` + `mint-token.ts`). The legacy verifier must parse `relm_`; it no longer accepts `relk_`.
...
2. **Lane isolation** — a `relk_` token is verified by Better Auth only; `relm_` by the legacy verifier only; the static key by exact compare only. No token is eligible for two verifiers, so there is no overlap-bypass and no double-meter.
...
- **Pure unit:** `scopeToPermissions` / `permissionsToScope` round-trip across `read`/`write`/`admin`; `requiredPermissionFor` route mapping; prefix routing (`relk_` vs `relm_` vs root vs garbage).
- **API middleware (`tests/db-helper.ts` fixtures):** valid user key grants per scope; insufficient scope → `403`; expired → `401`; rate-limit exceeded → `429`; quota exhaustion auto-disables then `401`; machine `relm_` key still works and is **not** metered; static root unaffected; self-serve create rejects `admin` scope.
...
3. Re-prefix the machine lane to `relm_`; confirm `internal`/`agent` paths + static root unaffected.
