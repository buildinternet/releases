...
| Touch fetch/parse/insert behavior               | [ingest.md](architecture/ingest.md), then [remote-mode.md](architecture/remote-mode.md) for the scheduling around it. Git-push ingest: [ingest.md → GitHub Action ingest](architecture/ingest.md#github-action-ingest) |
...
- **[semantic-alerts.md](architecture/semantic-alerts.md)** — freeform interest alerts (#2304): follows-only JEV matching, webhook and email delivery, the account preference UI, the admin match-rate summary, and the preview that inserts synthetic releases through `release.created`.
