...
Alerting on these (sign-in-failure spike, rate-limited surge, admin-action occurrence) is wired through Axiom monitors — validated APL + thresholds + response steps live in [docs/runbooks/auth-audit-monitors.md](../runbooks/auth-audit-monitors.md) (#1432). Monitors are created manually in the Axiom UI: there is no monitors-as-code path and the Axiom MCP can't create monitors.
