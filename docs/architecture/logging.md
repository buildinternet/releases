...
## AI provider health (`event: "ai_usage"`, `provider-quota-exhausted`)

Every AI lane emits an `ai_usage` event per model call (`component: "ai"`, with `provider`, `model`, `lane`, token counts and a cost estimate), and a provider-side quota or billing shutoff emits `provider-quota-exhausted` via `classifyProviderQuota`. Alerting on both — a quota shutoff, a single provider going dark with no error to show for it, inference stopping entirely, and releases inserting with nothing summarizing them — is wired through four **live** Axiom monitors; IDs, APL, backtests, and response steps live in [docs/runbooks/ai-provider-monitors.md](../runbooks/ai-provider-monitors.md) (#2168). The silence windows there are measured against real traffic, not chosen for roundness: read the tuning section before shortening one.
