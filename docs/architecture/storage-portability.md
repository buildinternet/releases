...
| Migrations            | hand-written SQLite `.sql`                | `apps/api/migrations/`                      | same set or subset            | parallel PG set (phase 2)                            |
...
| [`apps/api/src/queries/orgs.ts`](../../apps/api/src/queries/orgs.ts)                               | Org-scoped release filter (`MATCH ?` fragment)                                          |
| [`apps/api/src/queries/sources.ts`](../../apps/api/src/queries/sources.ts)                         | Source-scoped release filter (`MATCH ?` fragment)                                       |
...
([`apps/api`](../../apps/api/src/db.ts),
[`apps/mcp`](../../apps/mcp/src/db.ts),
[`apps/webhooks`](../../apps/webhooks/src/db.ts)) are thin re-exports, and
...
stays zod-free so it doesn't split `apps/mcp`'s SDK-pinned zod type graph, and it
...
[`apps/api/src/workflows/deterministic-update.ts`](../../apps/api/src/workflows/deterministic-update.ts)
...
2. ~~**Parameterize the D1 100-bound-param chunking**~~ (seam #6) — **done**
   ...
   `apps/api/src/lib/d1-limits.ts` which keeps the per-statement INSERT chunk sizes +
