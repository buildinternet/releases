...
| 6   | **D1 quirk assumptions**  | Small (parameterized)   | The 100-bound-parameter limit is now a named capability — `D1_MAX_IN_PARAMS = 90` in [`@releases/lib/db`](../../packages/lib/src/db.ts) — routed through by [`workers/api/src/queries/search.ts`](../../workers/api/src/queries/search.ts) and [`workers/api/src/queries/orgs.ts`](../../workers/api/src/queries/orgs.ts) instead of a bare `90`. Harmless on Postgres (far higher limit) and now raisable in one place. (`search.ts` deliberately _caps_ rather than chunk-unions — its FTS queries are ranked/`LIMIT`-ed, so a UNION across chunks would mis-rank; a single product owning >90 sources is not a served shape.) `.batch()`-chunked INSERTs (`floor(100 / binds_per_row)`) remain per-site by design — their divisor is row-shape-specific. |
...
2. ~~**Parameterize the D1 100-bound-param chunking**~~ (seam #6) — **done**
   ([#2009](https://github.com/buildinternet/releases/issues/2009)). `D1_MAX_IN_PARAMS = 90`
   now lives in `@releases/lib/db` (`createDb`-adjacent) and both `queries/orgs.ts` and
   `queries/search.ts` route through it, so the limit reads as a backend capability a
   higher-limit backend can bump in one place. `search.ts`'s `IN`-list cap was confirmed
   deliberate (ranked/`LIMIT`-ed FTS can't chunk-union) and documented as such.
