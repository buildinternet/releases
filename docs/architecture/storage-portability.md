...
All relational-DB construction in `workers/api` funnels through **one factory**,
`createDb` in [`workers/api/src/db.ts`](../../workers/api/src/db.ts):
...
Two known gaps in the funnel, both deliberately deferred (see follow-ups):

- The factory body is **duplicated** across three workers —
  [`workers/api/src/db.ts`](../../workers/api/src/db.ts),
  [`workers/mcp/src/db.ts`](../../workers/mcp/src/db.ts),
  [`workers/webhooks/src/db.ts`](../../workers/webhooks/src/db.ts). `workers/mcp` is a
  carved-out workspace (own `tsc`, excluded from root oxlint), so consolidating into a
  shared package is a cross-workspace change we haven't taken.
- [`packages/search/src/log-search.ts`](../../packages/search/src/log-search.ts) builds
  its own handle. A package can't import a worker's factory (backwards dependency) —
  this is a symptom that the factory lives in the wrong layer (see follow-up #1).
...
| #   | Seam                      | Effort                  | Why                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| --- | ------------------------- | ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Client / driver**       | Easy (done)             | Single `createDb` factory + `_drizzleOverride`. A Postgres backend branches here on `drizzle-orm/postgres-js` (or `node-postgres`).                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
- **Phase 1 — one factory, one layer.** Hoist the DB factory into a shared package
  (e.g. `@releases/db` or `packages/lib`) so `api`, `mcp`, `webhooks`, and
  `packages/search` all construct through it. This is the prerequisite that makes a
  backend branch a single edit.
...
1. **Hoist `createDb` into a shared package** so all four consumers (`api`, `mcp`,
   `webhooks`, `packages/search`) build through one factory — removes the 3× duplication
   and the `log-search.ts` boundary violation. This is the highest-value next step and
   is worth doing before any Postgres work.
