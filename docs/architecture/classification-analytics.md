...
The writer lives in `apps/api/src/lib/classification/classification-schema.ts`. Reads always filter `blob1 = '1'` and `blob4 = 'marketing'`. Semantic-alert decisions share this dataset with `blob4 = semantic-alert` and a separate writer; they stay out of the marketing admin queries. Their read API is `GET /v1/admin/semantic-alerts/summary` (match rate and fail-closed counts; cost stays on `ai_usage`). See [semantic-alerts.md](semantic-alerts.md).
