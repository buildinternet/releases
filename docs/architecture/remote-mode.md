...
Today's example: a `scrape` source's `source.url` points at the docs/changelog page, while `metadata.feedUrl` (auto-discovered on first add) is the actual RSS/Atom/JSON feed the cron polls and parses. Same principle should apply to other machine-friendly fetch backends — e.g. when a docs page is generated from a GitHub-hosted CHANGELOG, a `metadata.githubUrl` override would let us fetch from the repo while keeping `source.url` pointed at the docs page, with release URLs synthesized to anchor into that page.
