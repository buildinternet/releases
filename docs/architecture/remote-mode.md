...
This is the reference for the API worker at `apps/api/` — the heart of the backend. Every read and write in the system goes through it: the OSS CLI ([`buildinternet/releases-cli`](https://github.com/buildinternet/releases-cli)) is a pure HTTP client talking to `RELEASES_API_URL` (default `https://api.releases.sh`), the web frontend and MCP server are clients too, and the internal workers (MCP, discovery, webhooks, cron) bind directly to the same D1 database. There is no local-SQLite path anymore; "remote mode" — everything served from Cloudflare — is the only mode.
...
`apps/api/src/middleware/token-store.ts` (constant-time, uniform-failure) and
...
in `apps/api/src/auth/api-key-scope.ts`), enforced at both layers: the mint
...
(`apps/api/src/auth/entitlement.ts`: `user`→read, `curator`→read+write,
...
- `PATCH /v1/admin/users/role` `{ email | userId, role }` — set a role
  (`apps/api/src/routes/admin-users.ts`). The accepted role set is derived
...
- **Shared verifier** — `@releases/lib/oauth-jwt` (`isJwtShaped`,
  ...
  **better-auth-free**: `apps/mcp` must not import the AS (the zod-pin would
...
(`apps/api/src/lib/latest-cache.ts`) for 300 seconds; write-back on miss
...
`publicRateLimitMiddleware` (`apps/api/src/middleware/rate-limit.ts`) applies two independent Cloudflare Workers Rate Limiting bindings, each behind its own kill switch (both off by default, so initial deploys change nothing). Limit values live on the bindings in `wrangler.jsonc` — keep them out of user-facing docs. State is per-colo (CF constraint), not global. Wired only onto the public-read route group + `/graphql` in `apps/api/src/index.ts`, and only acts on safe (read) methods; admin routes are already key-gated.
...
Flip either var to `"true"` in `apps/api/wrangler.jsonc` and redeploy to activate.
...
The API Worker lives at `apps/api/` and shares the Drizzle schema from `@buildinternet/releases-core/schema` (`packages/core/src/schema.ts`). D1 migrations are in `apps/api/migrations/`. Deploy with `cd apps/api && wrangler deploy`.

## Migrations

D1 schema DDL lives exclusively in `apps/api/migrations/`. Prod applies it via `wrangler d1 migrations apply --remote` (automated in the API-worker deploy). Tests apply the same files via `tests/db-helper.ts` → `applyMigrations(sqlite)`, ensuring prod and tests share a single schema history. `schema.ts` is the source of truth for drizzle ORM types and drizzle-kit introspection; `drizzle-kit generate` exists as a scaffold only — its output is gitignored under `.drizzle-out/`.
...
Cron polling: the API Worker still runs an hourly `scheduled` handler, but it no longer fetches anything itself — per-source scheduling lives on the `SourceActor` Durable Object (below), and the hourly tick is a re-seed heartbeat for actors that have no pending alarm. Configure `GITHUB_TOKEN` as a Worker secret for GitHub source access. Tier intervals are controlled by `fetchPriority` on each source.

Workflows-based ingest (issue #486, follow-on to #482): every per-source fetch runs as a `POLL_AND_FETCH_WORKFLOW` instance, with step-level retries around `fetch-and-persist`, `embed-releases`, `refresh-changelog-file`, `embed-changelog-chunks`, and `invalidate-latest-cache`. The embed steps get 5 retries × 30s exponential — the whole reason for the migration is ride-out tolerance for Voyage 429s mid-source. `packages/search/src/embed-releases.ts` and `embed-changelog-pipeline.ts` accept an opt-in `throwOnError` so the workflow can actually observe failures; default callers stay fire-and-forget. Instances are `create()`d from each source's `SourceActor` alarm (the hourly cron no longer `createBatch`es them — see below), and the daily scrape-agent sweep workflow was retired with the OrgActor drain (#1822).

### SourceActor — per-source fetch scheduling (#1776)

A per-source Durable Object (`SOURCE_ACTOR.getByName(sourceId)`, `apps/api/src/source-actor.ts`) that owns a single source's fetch lifecycle: its own alarm-driven timer, tier cadence (normal 4h / low 24h), smart-fetch backoff, and single-threaded serialization. Every source self-schedules — the actor is the **sole per-source fetch driver**, and the hourly poll cron survives only as an idempotent re-seed heartbeat: it unions currently-due sources with active sources whose D1 mirror says the actor is unmanaged / missing `nextAlarmAt` (#2286) and calls `ensureScheduled` on each (idempotent when a live future alarm is already pending; a leftover _past_ `getAlarm()` is treated as missing so a dropped timer cannot no-op forever). Binding absent ⇒ nothing to drive — there is no cron-driven fetch fallback. **Division of labor: the DO owns the timer + mutex only — the proven `PollAndFetchWorkflow` still does all fetch/parse/embed/ingest, `create()`d from the alarm.** The actor never recomputes backoff: each alarm re-reads the source's D1 row and derives the next-due time from `computeFetchState()` (the same helper the dev fetch-plan panel uses), since the workflow already writes `last_polled_at` / `next_fetch_after` / `consecutive_no_change` to D1. Not-due ⇒ reschedule forward (alarms are ~free, so this is idempotent — a double-fire just re-reads D1); due ⇒ fire one ingest workflow + reschedule one interval out; `paused` / firecrawl (webhook-driven) / org-paused ⇒ stop. A never-polled source (`null last_polled_at`) fetches on its first alarm rather than waiting a full interval. Double-fires are prevented by an in-flight guard (a 15-min `SAFETY_WINDOW` longer than the worst-case ingest run) plus a workflow instance id bucketed by that window, so a spurious re-alarm collides on the id instead of spawning a second run. Admin fetch (`POST /v1/sources/:id/fetch` and `POST /v1/workflows/update`), unpause / tier PATCH, org unpause, and Firecrawl disable all force-arm the actor so `metadata.sourceActor` shows `managed: true` with a scheduled `nextAlarmAt`. Operators can also re-arm one source or sweep the unmanaged fleet via `POST /v1/workflows/rearm-source-actors` (`{ sourceId }` or `{ all: true }`; fleet sweep defaults to `dryRun: true`). `GET /v1/admin/sources/health` reports `meta.unmanagedActors`.

The rollout scaffolding is gone (#1819). The actor originally shipped behind the Flagship boolean `source-actor-enabled` plus a `SOURCE_ACTOR_COHORT_PCT` wrangler var, ramped to 100%, and then the cohort predicate (`isSourceActorManaged`), the flag, the var, and the cron's per-source workflow fan-out were deleted in one pass — rollback is a code revert + redeploy, not a flag flip (the same prove-then-delete treatment as the OrgActor and deterministic-update flags below). What survives of that module is the seed-jitter helper (`apps/api/src/lib/sources/source-actor-seed.ts`): a (re-)seeded first alarm is spread across a 5-min window so simultaneously-overdue sources don't thunder; after the first fetch each source's `last_polled_at` differs, so subsequent alarms land at distinct times (emergent jitter). D1 stays the system of record — DO storage holds only derived, rehydratable coordination state (re-read from D1 every alarm, so eviction is free); a best-effort write-through mirror into `metadata.sourceActor` (`{ managed, nextAlarmAt, lastAlarmAt }`, no new column/migration) keeps the dev fetch-plan panel showing actor status. The three per-source coordination bolt-ons that emulated a mutex on the **MA scrape/agent delegation path** (the KV `ma:active:src:*` lock, the update session-dedup window, and the `skipDelegation` guard) were subsequently retired onto the actor as well (#1780/#1814/#1816). Cross-source reconciliation (`ProductActor`) stays deferred (#1775/#1777). History: `docs/plans/durable-objects-exploration.md` (design exploration), `docs/superpowers/specs/2026-07-01-source-actor-scaffolding-removal-design.md` (removal spec); epic #1778.

### OrgActor — per-org scrape/agent drain (#1777)

Retires the `force-drain-sweep` (#518) producer and `scrape-agent-sweep` (#482) consumer crons by moving both jobs onto the actor model the `SourceActor` established above. A `type in (scrape, agent)` source gets flagged — `changeDetectedAt` set — inside the `PollAndFetchWorkflow` poll step (`pollOne`) when its `changeDetector` reports a change, or self-flagged when the detector is `unreliable`/`absent` or the source is stale (`lastFetchedAt` older than `FORCE_DRAIN_STALE_HOURS`, default 72h), which is the same signal the retired force-drain-sweep used to set on a cron tick. **The producer arms `OrgActor` the instant it sets the flag** (#1946 phase 2): `pollOne` calls the shared `notifyOrgDrain` (`apps/api/src/lib/sources/org-drain-notify.ts` → `ORG_ACTOR.getByName(orgId).ensureDrainScheduled(orgId)`, an in-worker DO RPC) as it finishes, so the drain arms within the same alarm that flagged the source — no arming lag. `SourceActor.alarm()` keeps a second, backstop notify driven off the `Source` row it loads at the **top** of `alarm()`: on the alarm whose own workflow just flagged the source the eager path already armed the drain, and this re-arms on every subsequent alarm while the flag persists — the at-least-once safety net for a stranded source whose eager notify was dropped. `ensureDrainScheduled` is idempotent (arms a jittered alarm on the per-org `OrgActor` DO — `apps/api/src/org-actor.ts` — only when none is pending), so the eager + backstop notifies firing for the same flag is harmless. Before #1946 phase 2 only the backstop existed, so first arming could lag the flag by up to one tier interval (4h/24h). On its own alarm, `OrgActor` queries its org's flagged scrape/agent candidates (the same filter the retired sweep used: not paused/hidden/firecrawl/feed, org not `fetch_paused`) and dispatches a single deterministic update run for the batch via `startDeterministicUpdate` (`apps/api/src/lib/sources/update-dispatch.ts`, #1946) — one run per org per drain pass, same as the sweep. The `OrgActor` holds no budget or lock logic of its own: the shared dispatch gate enforces the kill switch, the per-org ($2/day) and global ($15/day) spend cap (`checkSpendCap`, counters in the shared `LATEST_CACHE` KV namespace), and the per-source scrape lock (#1815) before creating the workflow instance, so a refused dispatch (cap hit or locked) is logged and dropped — the source stays flagged and re-drains on the next `SourceActor` notify, since the recurring poll alarm is the at-least-once safety net (no markers table needed).

This is the only scrape/agent drain path. It originally shipped behind the Flagship boolean `org-drain-actor-enabled`, with the legacy `force-drain-sweep` (#518) + `scrape-agent-sweep` (#482) crons as the flag-off fallback; once the actor path proved stable in prod both crons and the flag were retired (#1822/#1888), so the actor path now runs unconditionally and rollback is a code revert + redeploy rather than a flag flip. The per-source scrape lock (#1815) guards against double-dispatch. The candidate query the `OrgActor` uses now lives in `apps/api/src/lib/sources/drain-candidates.ts` (`queryCandidates`), relocated out of the deleted sweep cron. Design: `docs/superpowers/specs/2026-07-01-orgactor-drain-coordinator-design.md`; plan: `docs/superpowers/plans/2026-07-01-orgactor-drain-coordinator.md`.

#### Deterministic update runs (#1878 → #1946)

A routine update is a deterministic fetch→extract pipeline: every real decision (URL, crawl vs render, incremental vs seed, dedup, retry) lives in `scrapeFetch` (now `packages/adapters/src/scrape-fetch.ts`, shared with onboarding's `manage_source(fetch)` custom tool). #1878 removed the Haiku worker-agent shell (a fixed ~19k-token prompt+skills+playbook cache-creation tax, ~84% of session cost) but left the loop running inside the discovery worker's `ManagedAgentsSession` DO behind the `POST /update` service boundary. #1946 phase 1 removed that boundary too: update runs now execute as the API worker's **`DeterministicUpdateWorkflow`** (`apps/api/src/workflows/deterministic-update.ts`), one `step.do` per source, dispatched through the shared `startDeterministicUpdate` gate (`lib/sources/update-dispatch.ts`: kill switch → spend cap → per-source lock → `create()`), and `apps/discovery` serves **onboarding only**. All three dispatchers use the gate: the OrgActor drain above, `POST /v1/workflows/update` (unchanged wire contract for the CLI: 202 `{sessionId, status: "running"}` / 409 + `Retry-After` / 429 / 503), and the poll-fetch summary-only crawl-feed delegation (which replaced the retired `startManagedFetchSession` RPC; a refused dispatch there stamps a short `nextFetchAfter` cooldown so the source doesn't re-attempt delegation every poll tick). Runs still register with StatusHub (`sessionType: "update"`, `agent: "deterministic"`, id prefix `det-`), so `/v1/sessions/:id` polling is unchanged; per-source extraction sub-calls (incremental Haiku, or the DeepSeek/OpenRouter tool-loop) self-log `ai_usage` — there is no session-level spend to record. `scrapeFetch` still persists through the HTTP API surface, via the `API_SELF` self service binding (same per-call invocation semantics as the old discovery→api hop; direct-DB persistence is the phase-3 consolidation). The workflow requires the scrape secrets (`CLOUDFLARE_ACCOUNT_ID` + `CLOUDFLARE_API_TOKEN`) and fails the run terminally when they're absent (local/misconfig). The old DO-alarm wall-clock budget (12 min, skip-the-rest) is gone — each source is its own step with a 15-min timeout, so every source in the batch runs. A batch where every source fails is still reported as a terminal session failure. #1946 phase 2 landed the eager `notifyOrgDrain` that removes the OrgActor arming lag (documented above). Remaining phase of #1946: the Firecrawl fold-in.
...
A second daily cron at 03:00 UTC (`apps/api/src/cron/retier.ts`) recomputes `fetchPriority` from the median `publishedAt` gap in the last 180 days: ≤14d → normal, 14-90d → low, >90d preserves the current tier. Never auto-pauses (manual vs automatic overrides aren't tracked yet), never touches sources that are already `paused`, and skips tier changes for sources with <3 releases of signal. The retier persists its signal on every source it evaluates via `sources.medianGapDays` (REAL; null when <3 releases of signal) and `sources.lastRetieredAt` (ISO timestamp); the API returns both on `GET /v1/sources`, and the dev-gated status dashboard (`apps/web/src/app/admin/status/`) renders them as a Cadence column that flags mismatches between cadence and tier (e.g. a paused source still shipping on a 5-day median). The `lastPolledAt` column tracks when each source was last polled by the cron.
