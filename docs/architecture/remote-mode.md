...
A routine update is a deterministic fetch→extract pipeline: every real decision (URL, crawl vs render, incremental vs seed, dedup, retry) lives in `scrapeFetch` (`packages/adapters/src/scrape-fetch.ts`, shared with the local-ingest skill's fetch step). #1878 removed the Haiku worker-agent shell (a fixed ~19k-token prompt+skills+playbook cache-creation tax, ~84% of session cost) but left the loop running inside the (now-retired) discovery worker's `ManagedAgentsSession` DO behind a `POST /update` service boundary. #1946 phase 1 removed that boundary too: update runs now execute as the API worker's **`DeterministicUpdateWorkflow`** (`apps/api/src/workflows/deterministic-update.ts`), one `step.do` per source, dispatched through the shared `startDeterministicUpdate` gate (`lib/sources/update-dispatch.ts`: kill switch → spend cap → per-source lock → `create()`). All three dispatchers use the gate: the OrgActor drain above, `POST /v1/workflows/update` (unchanged wire contract for the CLI: 202 `{sessionId, status: "running"}` / 409 + `Retry-After` / 429 / 503), and the poll-fetch summary-only crawl-feed delegation (which replaced the retired `startManagedFetchSession` RPC; a refused dispatch there stamps a short `nextFetchAfter` cooldown so the source doesn't re-attempt delegation every poll tick). Runs still register with StatusHub (`sessionType: "update"`, `agent: "deterministic"`, id prefix `det-`), so `/v1/sessions/:id` polling is unchanged; per-source extraction sub-calls (incremental Haiku, or the DeepSeek/OpenRouter tool-loop) self-log `ai_usage` — there is no session-level spend to record. `scrapeFetch` still persists through the HTTP API surface, via the `API_SELF` self service binding (same per-call invocation semantics as the old discovery→api hop, now a same-worker self-call). The workflow requires the scrape secrets (`CLOUDFLARE_ACCOUNT_ID` + `CLOUDFLARE_API_TOKEN`) and fails the run terminally when they're absent (local/misconfig). The old DO-alarm wall-clock budget (12 min, skip-the-rest) is gone — each source is its own step with a 15-min timeout, so every source in the batch runs. A batch where every source fails is still reported as a terminal session failure. #1946 phase 2 landed the eager `notifyOrgDrain` that removes the OrgActor arming lag (documented above). The Firecrawl fold-in (#1946's remaining phase) and the retirement of the discovery worker itself have since landed — onboarding now runs through Claude Code sessions (see [local-ingest.md](local-ingest.md)) rather than a hosted worker.
