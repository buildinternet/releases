...
| -------------------------------------- | ------- | -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `agent-trace-payloads-enabled`         | `false` | api            | Record message + tool payloads on Cloudflare Agents AI-SDK spans (session replay). Off = metadata-only spans (tokens, model, duration). Leave off unless actively debugging.                                                                          |
...
| `cache-disabled`                       | `false` | api            | Kill switch for `Cache-Control` response headers, which drive Workers Cache at the edge. Slow-drain: flipping true stops new entries; already-cached responses age out over their remaining `max-age`. false = caching active.                        |
...
| `listing-self-serve-enabled`           | `true`  | api            | Anonymous self-serve listing lane (/v1/listing validate + activate). Off = both routes refuse.                                                                                                                                                        |
| `listing-self-serve-promotion-enabled` | `false` | api            | Self-serve Tier-1 promotion for verified owners (/v1/listing/promote). Off = route refuses. Creates live fetching sources, so this is a genuine incident lever distinct from the zero-cost listing lane.                                              |
...
| `semantic-alerts-enabled`              | `false` | api            | Semantic alerts (#2304). Off = /v1/me/semantic-alerts 404, the account UI hides the section, and the release matcher does not run. On = preferences plus follows-only JEV matching with webhook and email. Structural follow webhooks are unaffected. |
| `web-bot-auth-enabled`                 | `false` | api, discovery | Signs outbound content fetches with RFC 9421 Web Bot Auth signatures.                                                                                                                                                                                 |
| `well-known-materialization-enabled`   | `true`  | api            | Owner-manifest product/source creation. Off = reconcile and mark existing entities only.                                                                                                                                                              |
