...
- `GET /v1/me/settings/notifications` — one-shot bootstrap for `/account/notifications`: `{ cadence, feedToken, webhooks, semanticAlerts }` (read-only; writes stay on `/me/digest`, `/me/feed/token`, `/me/webhooks`, `/me/semantic-alerts`). `semanticAlerts` is `null` when `semantic-alerts-enabled` is off. Same principal gate as follows. Web RSC hydrates via cookie-forward; the panel falls back to a single client fetch when SSR is empty.
...
- `/v1/me/semantic-alerts` — freeform interest alerts (`GET/POST`, per-id `GET/PATCH/DELETE`), max 5 per account, threshold default 0.80. Same principal gate as follows. `semantic-alerts-enabled` off → 404. Phase 1 stores the query and email/webhook preferences only; the candidate pool is follows-only and nothing is scored yet. `GET /v1/me/settings/notifications` includes `semanticAlerts` (`null` when the flag is off). See [semantic-alerts.md](semantic-alerts.md).
...
`GET /v1/lookups/by-domain?domain=…` resolves a normalized domain to its owning org (matched on `organizations.domain` for primary or `domain_aliases.domain` for aliases) and any products whose alias targets the same domain. On a complete miss, when `listing-self-serve-enabled` is on, the handler may **just-in-time** probe `https://{domain}/.well-known/releases.json` and materialize a stub via `createStubFromManifest` (#2030) — the same core as listing activate and the nightly `domainDemandSweep`. Outbound probes use the registered bot UA (`releases/0.1 (+https://releases.sh)`) and `makeBotFetch` (Web Bot Auth when enabled). Per-domain budget: 3 probes / ~10 min (KV `lookup:domain-manifest:{domain}`); per-IP limiter on the outbound branch only (`domain-jit:{ip}`). Budget exhaustion → `404`; IP over-limit → `429`. Misses still record `domain_demand` so the cron remains a backstop. Mirrored on MCP as `lookup_domain` and on the OSS CLI as `releases lookup domain <domain>`. The normalizer `normalizeDomain` (`@buildinternet/releases-core/domain`) is shared with `/v1/search?domain=…` and the MCP `search` tool's `domain` input to scope a query to one org by URL-shaped input.

Org detail (`GET /v1/orgs/:slug`) also accepts a bare domain: after id/slug and alias matching, a domain-shaped identifier is matched against `organizations.domain` with the same primary-or-alias semantics as `by-domain` (both share `normalizeDomain`). On a miss it records a fire-and-forget **domain-demand** signal (`domain_demand` table, #1947) — the same signal `by-domain` emits — so the daily manifest sweep can later probe undeclared domains that people are already asking for. This is a demand _record_ only: `org get` does **not** run in-request JIT (that stays on `by-domain`).
...
## Importance filtering — `minImportance`
