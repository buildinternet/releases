...
(`requireSession`), are absent from both `publicReadRoutes` and `adminRoutes`, and
use the same credentialed CORS carve-out in `index.ts`. They are gated behind the
`user-follows-enabled` Flagship flag. Endpoints:
