...
- `firecrawl-staleness` — `scan-complete` (`scanned`, `stale`), `stale-source` (per flagged row).
- `source-staleness` — `scan-complete` (`scanned`, `stale`), `stale-source` (per flagged row).
- `push-staleness` — `scan-complete` (`scanned`, `stale`), `stale-source` (per flagged row) — push-fed sources gone quiet (#2381).
- `staleness-digest` — `skipped-empty` (no overdue sources), `email-sent`, `email-skipped`, `email-error` (daily rollup via `sendStalenessDigest`).
...
**Universal capture on the steady-state scrape path (#1283).** Beyond deep backfills and the Firecrawl webhook, the everyday scrape→extract path captures raw too — gated behind `raw-snapshot-capture-enabled` (default off). `runScrapePath` acquires the markdown (markdownUrl / crawl / Cloudflare Browser Rendering); once the body is finalized it POSTs it to `POST /v1/orgs/:orgSlug/sources/:sourceSlug/raw-snapshot` (scrape-only), which calls `saveRawSnapshot`. The call is best-effort (`try/catch` + warn) and never aborts extraction. Content-hash dedup means an unchanged page does not re-store, and the captured body is exactly what extraction consumed — so it can be reprocessed faithfully.
...
**Single-release in-place re-fetch.** `POST /v1/workflows/refetch-release { releaseId, url?, dryRun?, force? }` heals ONE release from its live page without minting a new `rel_` id (the delete-and-refetch workaround kills web links): it fetches the post as markdown (Firecrawl `scrapeOnce` when the source has `firecrawl.enabled`, else plain fetch + `htmlToMarkdown` — SSR/static pages only), runs the single-post body-preserving extraction (`extractFirecrawlMarkdown` with `pageUrl`, i.e. `CRAWL_PAGE_SYSTEM_PROMPT`, Haiku), and updates the row in place. Curator semantics, unlike ingest's fill-if-empty: title/content/publishedAt are **replaced**, content sizes + `contentHash` recomputed, and `summary`/`titleGenerated`/`titleShort`/`embeddedAt` nulled so the AI passes regenerate; media is replaced only when extraction returns items (a miss never wipes curated images), mirrored through `processMediaForR2`. A stored `#fragment` URL (synthesized index anchor) requires an explicit `url` — fetching it would ingest the index — and the override must stay on the source's host; when given, it **rewrites** the stored URL to the canonical permalink (409 on a `UNIQUE(source_id, url)` collision). `dryRun` (default) previews current vs. proposed. Two write guards (#2077): placeholder extraction content (`<UNKNOWN>`, `n/a`, … — the same `PLACEHOLDER_RE` set ingest strips from versions) is a 502, never written; and a write that would shrink a ≥200-char body below 50% of its current size is refused with a 400 unless `force: true` (dry-run flags it as `wouldRequireForce` instead). Built for thin index-scrape rows left behind by a crawl-mode flip (#2013's "history doesn't self-heal" gap).
...
Firecrawl-owned sources are excluded (covered by `scanStaleFirecrawlSources`); push-fed sources are excluded too — they have their own, quieter scan (below). Emits `warn`-level `stale-source` events on the `source-staleness` component; no DB mutation, no cost, no feature flag.

**Push-fed staleness scan (`cron/push-staleness.ts`, #2381).** Push-fed sources (`metadata.ingestMode: "push"`, #2378 — fed directly by their publisher, typically `actions/publish-changelog` POSTing to `POST /v1/sources/:id/releases/batch` on every push) have no poll cadence and are excluded from both the poll cron and the scan above (`locallyFetchedSql`), so nothing else notices when a publisher's pipeline quietly breaks (a rotated/expired API token, a disabled workflow, a `paths:` filter that no longer matches, a repo rename). `scanStalePushFedSources`, riding the same `0 4 * * *` tick, flags a push-fed, non-deleted, non-hidden, non-paused source whose **last activity** — the later of `lastFetchedAt` (stamped by the batch route on every successful push, including a no-op re-run) and the newest non-suppressed release date, falling back to `createdAt` — is older than its overdue window. The window reuses the first-party scan's exact knobs, `max(SOURCE_STALE_FLOOR_DAYS=14, medianGapDays × SOURCE_STALE_MULTIPLIER=3)` — no new env var. Unlike the first-party scan, a push-fed source with no established cadence (`medianGapDays == null`) is **not** skipped: a broken pipeline on a new source still matters, so it uses a fixed 30-day (`NO_CADENCE_WINDOW_DAYS`) window instead. Emits `warn`-level `stale-source` events on the `push-staleness` component.

**Operator digest (`cron/send-staleness-digest.ts`).** The same `0 4 * * *` tick runs `sendStalenessDigest` after all three scans. It reuses each scan's `entries[]` output, builds a combined text+HTML rollup (`lib/email/staleness-digest-email.ts`), and sends via the `SEND_EMAIL` binding (`EMAIL_NOTIFY_TO`) when the actionable count (`countNeedsAttention`) is > 0 — opaque first-party sources, Firecrawl monitors, provider-health entries, and push-fed sources all count; upstream-quiet transparent sources (GitHub/App Store/video) ride along informationally and never trigger an email alone. Push-fed entries render under their own "No pushes lately" section, worded as "check the publisher's pipeline" rather than "scrape overdue". Empty rollups log `staleness-digest/skipped-empty` and send nothing — quiet days stay quiet. Preview the template from `/admin/emails` (`operator.staleness-digest` sample) or send a fabricated test via `POST /v1/admin/emails/test`.
