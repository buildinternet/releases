...
**Non-source pages use a generalized sibling, `notifyWebRevalidatePaths` (#2331).** The homepage "Weekly digests" reel, each collection's latest-digest hero, and the `/collections` index (it shows per-row digest recency) aren't source-shaped, so they can't be derived from `{ orgSlug, sourceSlug, productSlug }`. After the weekly-digest generation writes at least one new digest, `pingAfterDigests` (`workers/api/src/cron/collection-summaries.ts`) sends one ping listing `/`, `/collections`, and `/collections/<slug>` for each collection that got a digest; a run with zero new digests sends none. Both the live path (`CollectionSummariesWorkflow.runWeeklyDigests`, wrapped in its own `step.do` so a workflow replay can't double-send) and the inline `runCollectionWeeklyDigests` fallback call the same helper. Web's route accepts this as a new `{ paths: string[] }` body, validated against a path allowlist and capped at 50 entries, alongside the unchanged `{ orgSlug, ... }` shape.
