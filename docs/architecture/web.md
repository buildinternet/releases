...
**Semantic alerts preview** (`POST /v1/admin/semantic-alerts/preview`, purge via `POST /v1/admin/semantic-alerts/purge`) inserts synthetic changelog rows through the real batch publish path so the Phase 2 matcher can be demoed. The dedicated `semantic-alerts-demo` org is the default target. Passing `userId` on that source follows the demo org for the account; purge removes the synthetic releases and leaves the follow. See [semantic-alerts.md](semantic-alerts.md).
