...
**Interest alerts** (`SemanticAlertsSection` on `/account/notifications`) list, create, enable, disable, and delete freeform semantic alerts when `GET /v1/me/settings/notifications` returns a `semanticAlerts` array. `null` (flag off, the default) hides the section. Matches are delivered by email and webhook; the panel does not list them. See [semantic-alerts.md](semantic-alerts.md).
