...
This monorepo consumes `@buildinternet/releases-lib` and `@buildinternet/releases-skills` like any other npm dependency. It does **not** consume `@buildinternet/releases-core`: the monorepo has its own superset private workspace at `packages/core/`, imported as `@releases/core-internal`, which holds the DB-coupled pieces the thin OSS CLI doesn't need. See the "Carved-out packages" section in `AGENTS.md` and the consolidation plan in #370 for the contract between the two.

The monorepo does not carry `npm/*` scaffolds, Changesets config, or a `Release CLI` workflow — if you see those in an old PR, don't restore them.
