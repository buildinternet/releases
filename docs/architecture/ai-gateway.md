...
What the code sends today: a static `trace` block with `generation_name` (`"marketing-classifier"` / `"summarize-release"` / `"feed-enrich"`, or `"summarize-eval"` / `"feed-enrich-eval"` from the local evals) and `environment` (the worker's `ENVIRONMENT` var). No prompt/completion content is in the trace block — only labels — so it is never a PII surface on its own.
...
**Marketing decisions.** `resolveMarketingModel` selects the `DecisionModel` seam for `typesafe/jev-1.13`, using the same gate, credentials, lane override, app headers and Broadcast tags. OpenRouter provider 3.1 exposes `evaluationModel()`; AI SDK `experimental_evaluate` sends typed choices to `/api/alpha/decisions`, never `/chat/completions`. JEV calls emit the same `ai_usage` fields, with provider-reported tokens/cost and zero cache counters. Missing configuration falls back to the existing Anthropic text classifier; runtime decision failures keep releases visible without a second model call. The suppression threshold is selected-choice probability 0.80; see [ingest.md](ingest.md#marketing-classifier).

The JEV `ai_usage` event additionally includes `decision.choice`, `decision.selectedChoiceProbability` (the probability for that choice only), and `decision.providerConfidence` (the separate provider confidence signal). Missing or nonfinite scores are omitted. This matches the classifier diagnostics exposed as `result.decision` by the manual marketing lane, so kept marketing choices can be distinguished from safe choices without storing diagnostics in release rows. Text lanes retain their existing telemetry shape.
