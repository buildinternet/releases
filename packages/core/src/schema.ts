...
export const releases = sqliteTable(
  "releases",
  {
    ...
    // AI-scored newsworthiness (1-5, nullable), generated by the same Haiku
    // release-content pass as summary/title_generated/title_short. Fail-open:
    // null when unclassified (no model call, parse miss) or not yet backfilled.
    // No default — distinct from `breaking`'s "unknown" sentinel, since there
    // is no meaningful non-null default for an importance score.
...
export const releasesVisible = sqliteView("releases_visible", {
  ...
  migrationNotes: text("migration_notes"),
  // Type-level mirror only, same caveat as `breaking` above — the real view
  // already exposes this column (added by 20260712000000_add_release_importance.sql).
  importance: integer("importance"),
  url: text("url"),
  contentHash: text("content_hash"),
  contentChars: integer("content_chars"),
