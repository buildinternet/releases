...
| Add source            | `releases admin source add <name> --url <url> [--type <type>] [--org <org>] [--feed-url <url>] [--primary]`                       | `manage_source` action "add" with name, url, type, organization, feed_url, **is_primary** (type auto-detected if omitted; set is_primary=true on add to promote in one call — see "Primary Sources") |
...
**Set `is_primary` on the `add` call. Do not add first and edit after.** One call, not two:
...
When onboarding a new org with a single top-level changelog, this is always the pattern. Only use `manage_source(action="edit", is_primary=true)` when promoting a source you added in a prior session — never in the same flow as the add.
