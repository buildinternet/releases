...
Structure:
1. Open with one concrete sentence naming a specific recent ship — at most 25 words, and exactly one sentence. Name the actual feature(s); don't summarize at the category or platform level, and don't lead with a version range. If several ships compete to lead, name the biggest one here and give the others their own sections — don't let the opener grow into a paragraph of minor items. The opener follows the same self-reference rule as the rest of the body (see Guidelines).
   - Good: "Shipped realtime voice in the AI SDK and signed URLs for Blob storage."
   - Good: "The connection pool was rewritten, cutting steady-state memory on busy services."
   - Bad (vague / categorical): "Improved deployment resilience across the platform." / "The v1.3.x series shipped test infrastructure, native APIs, and package-manager work."
   - Bad (org name as subject): "Vercel shipped a new gateway." / "Bun's recent focus is testing."
2. Two to five themed sections — **five is a hard cap**. Every primary section opens with a **bold tease**; never write the page as one running paragraph. Each section uses one of two shapes:
   - **Bold tease** + a tight bullet list of concrete items.
   - **Bold tease** + one to two short prose sentences (each ≤25 words).
   Sections with three or more concrete items MUST bullet — don't pack them into a comma-separated paragraph. A prose sentence with four or more comma-listed items is the tell. A bullet that itself enumerates a small set ("works with A, B, C, and D") is fine. If more than five surfaces shipped, consolidate the smaller ones into a single wrap-up section or bullet group rather than adding a sixth.
3. The bold tease states a user-facing capability — never a version number, command name, endpoint, parameter, internal class name, or bare category label as the headline noun. Paired rewrites:
   - Good: "**The test runner gained CI-grade isolation and sharding.**" Bad: "**`bun test` gained `--isolate` and `--shard`.**"
   - Good: "**Exports can now target PDF and CSV.**" Bad: "**`POST /v2/exports` accepts a `format` param.**"
   - Good: "**Linear Agent gained MCP context reach.**" Bad: "**Linear Agent v2.4 added /mcp endpoint.**" / "**Platform and API.**"
   - Good: "**Now runs on Postgres 18 — `MERGE ... RETURNING` and virtual generated columns.**" Bad: "**Postgres 18 support.**" (a bare version/platform label is not a capability).
   Versions, commands, and code carry weight in supporting prose or bullets — just not as the lead.
...
Guidelines:
- Past tense, active voice for ship verbs — "shipped", "added", "removed", "rewrote". Use past tense for the ship itself, not a present-tense passive: "was rewritten" or "rewrote", not "is rewritten". Present tense is fine when describing what a shipped feature does ("the new endpoint accepts JSON"). No progressive forms about the org ("is shipping", "has been improving").
...
- **No editorializing about strategy or impact, and no narrating design intent.** State what shipped and what it does; don't grade it or explain the rationale behind it. "Further improving developer experience", "doubling down on AI", "leap forward", "powerful new direction", "pushing forward", "clear edge", "balances reliability and cost", "with conservative thresholds" — all fail. Drop bare intensity adverbs that grade a change ("significantly", "dramatically", "substantially") — state the concrete amount instead.
...
- **Structure even a short page.** Give each distinct shipped surface its own bold-tease section with its concrete details; don't compress two separate features into one sentence. Two solid features are two sections, not one line. (Don't pad to reach the floor, though — if signal is genuinely thin, a shorter page with fewer sections is right.)
- When updating an existing page, preserve still-relevant context. Condense or drop older themes that are no longer the focus. Don't rewrite from scratch — amend and evolve. But the opener and every bold tease in the result must still satisfy the rules above: if the existing page opens with the org name or a version range, or uses a version / command / endpoint as a headline, rewrite those lines — don't carry a violation forward just because it was already there.
...
- Hard floor: 80 words. Target 120–250 words; consolidate sections rather than exceed 250. Shorter only if signal is genuinely thin. Hard ceiling: 300 words.
