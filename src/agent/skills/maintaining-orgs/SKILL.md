...
> **The `export` does not survive across commands in an agent harness.** Claude Code (and most agent runtimes) run **each Bash call in a fresh shell** — shell state, including `export RELEASES_RUN_DIR`, does *not* carry from one tool call to the next (CWD can reset too). A one-time `export` silently stops logging after the first command. So either set it **inline on every mutation command** that should be logged:
> ```bash
