...
server.registerTool(
  ...
  {
    description:
      "Search indexed release notes. In local mode this is FTS-only; the `mode` parameter is accepted for compatibility but always degrades to lexical.",
    inputSchema: {
      ...
      mode: z
        ...
        .optional()
        .describe(
          "Retrieval strategy. Accepted for parity with the remote MCP server; local mode always runs lexical FTS.",
        ),
      include_coverage: z
        .boolean()
        .optional()
        .describe(
          "Include releases grouped as coverage of another (e.g. marketing posts that re-announce a platform release). Defaults to false.",
        ),
    },
  },
  async ({
    ...
    let results = searchReleases(query, needsPostFilter ? maxResults * 5 : maxResults, {
      includeCoverage: include_coverage === true,
    });
...
server.registerTool(
  ...
  {
    description:
      "Search orgs, products, and sources in the registry. In local mode this is the LIKE-based unifiedSearch helper — there is no semantic path available without a Vectorize binding.",
    inputSchema: {
      query: z.string().describe("Search query"),
      kind: z
        .enum(["org", "product", "source"])
        .optional()
        .describe("Restrict to one entity kind. Omit for all."),
...
server.registerTool(
  ...
  {
    description:
      "Read the full AI-generated overview for an organization — a short briefing that distills recent changelog activity into themed sections. Returned with a generated-at timestamp and a stale warning if the overview is older than 30 days.",
    inputSchema: {
      identifier: z.string().describe("Organization slug, domain, name, or account handle"),
    },
  },
...
server.registerTool(
  ...
  async ({ source: identifier, path: requestedPath, offset, limit, tokens }) => {
    ...
    if (!selected) {
      ...
      return textResult(
        `No CHANGELOG file found at path "${requestedPath}" for "${source.slug}". Available files:\n${available}`,
      );
    }
...
if (isAdminMode()) {
  ...
  server.registerTool(
    "add_source",
    {
      ...
      inputSchema: {
        ...
        organization: z
          .string()
          .optional()
          .describe("Organization slug, name, or domain to associate"),
      },
    },
    async ({ name, url, type, slug, organization }) => {
      ...
      if (exclusion.excluded) {
        const label =
          exclusion.scope === "blocked" ? "globally blocked" : "ignored for this organization";
        return textResult(
          `URL is ${label}: ${url}${exclusion.reason ? ` (${exclusion.reason})` : ""}`,
        );
      }
    ...
    "fetch_source",
    {
      ...
      inputSchema: {
        ...
        force: z
          .boolean()
          .optional()
          .describe("Delete existing releases before fetching (clean re-fetch)"),
      },
    },
    async ({ slug, force }) => {
      const db = getDb();
    ...
    "add_organization",
    {
      ...
      inputSchema: {
        ...
        description: z
          ...
          .optional()
          .describe(
            "Brief product description, one sentence (e.g. 'Event-driven durable workflow engine for TypeScript')",
          ),
      },
    },
    ...
    "block_url",
    {
      ...
      inputSchema: {
        ...
        type: z
          .enum(["exact", "domain"])
          .optional()
          .describe("Block type: exact URL or entire domain (default: exact)"),
  ...
  server.registerTool(
    ...
    {
      description:
        "Suppress a release so it no longer appears in queries or search results. Useful for filtering out promotional content or non-changelog entries from an otherwise good source.",
      inputSchema: {
        id: z.string().describe("Release ID to suppress"),
        reason: z
          .string()
          .optional()
          .describe("Why this release is being suppressed (e.g. 'promotional content')"),
      },
    },
    async ({ id, reason }) => {
