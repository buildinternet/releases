  ...
  program
    ...
    .action(async (slug: string, opts: { json?: boolean }) => {
      ...
      for (const r of results) {
        ...
        console.log(
          chalk.green(`${verb} ${r.path} for ${source.slug} (${r.bytes} bytes${suffix})`),
        );
      }
      if (pruned > 0) {
...
export function registerChangelogCommand(program: Command) {
  program
    ...
    .option(
      "--tokens <n>",
      "Target slice size in tokens (cl100k_base). Overrides --limit. Common brackets: 2000/5000/10000/20000",
      (v) => parseInt(v, 10),
    )
    .option(
      "--json",
      "Output as JSON with offset, nextOffset, totalChars, totalTokens, sliceTokens",
    )
    .action(
      async (
        slug: string,
        ...
        if (isRemoteMode()) {
          ...
          if (allRows.length === 0) {
            logger.error(
              `No CHANGELOG file is tracked for ${source.slug}. Run 'releases admin source refresh-changelog ${source.slug}' first.`,
            );
            process.exit(1);
          }
          ...
          if (opts.path) {
            ...
            if (!match) {
              logger.error(
                `No CHANGELOG file at path "${opts.path}" for ${source.slug}. Available: ${allRows.map((r) => r.path).join(", ")}`,
              );
              process.exit(1);
            }
            selected = match;
