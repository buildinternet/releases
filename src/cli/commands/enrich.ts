...
export function registerEnrichCommand(program: Command) {
  program
    ...
    .option("--force", "Bypass triage and re-enrich all candidates")
...
Examples:
  ...
  releases enrich sentry-changelog --force      Bypass triage, re-enrich all
  releases enrich sentry-changelog --json       Machine-readable output`)
    .action(async (slug: string, opts: { dryRun?: boolean; limit?: string; force?: boolean; json?: boolean }) => {
...
function printResult(result: EnrichResult, slug: string, elapsed: string, dryRun?: boolean) {
  ...
  if (result.releases.some((r) => r.status === "enriched")) {
    ...
    for (const r of result.releases.filter((r) => r.status === "enriched")) {
      const mediaSuffix = r.mediaAdded
        ? chalk.dim(` (+${r.mediaAdded} media, ${r.mediaTotal} total)`)
        : r.mediaTotal
          ? chalk.dim(` (${r.mediaTotal} media)`)
          : "";
      console.log(`    ${chalk.green("✓")} ${stripAnsi(r.title)}${mediaSuffix}`);
    }
  }
