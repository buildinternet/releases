...
export function registerSummarizeCommand(program: Command, opts?: { commandName?: string }) {
  program
    ...
    .action(
      async (
        slug: string,
        opts: { monthly?: boolean; window?: string; json?: boolean; force?: boolean },
      ) => {
        ...
        if (!opts.force) {
          ...
          if (!enabled) {
            console.error(
              chalk.yellow(`Summarization is disabled for ${slug}. Use --force to override.`),
            );
            process.exit(1);
          }
        }
        ...
        if (opts.monthly) {
          ...
          if (monthlyReleases.length === 0) {
            console.error(
              chalk.yellow(
                `No releases found for ${lastMonth.toLocaleDateString("en-US", { month: "long", year: "numeric" })}`,
              ),
            );
            process.exit(0);
          }
          ...
          logger.info(
            `Generating monthly summary for ${source.name} (${monthName}, ${monthlyReleases.length} releases)...`,
          );
          ...
          logger.info(
            `Generating rolling summary for ${source.name} (${recentReleases.length} releases, ${windowDays}-day window)...`,
          );
