Recent work converged on the `7.8.0` release — CLI bootstrap, query-cache controls, and partial indexes.

**Prisma Postgres setup is now a single interactive command.** `prisma bootstrap` (added in `7.7.0`) detects project state and runs only the steps you need:

- scaffold from one of ten starter templates
- link via browser auth
- install, migrate, generate, and seed

`7.6.0` added `prisma postgres link` as the first member of a new `prisma postgres` CLI group.
