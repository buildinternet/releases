...
describe("CLI org tags", () => {
  ...
  it("removes a tag", () => {
    const result = cli(dataDir, ["admin", "org", "tag", "remove", "acme-corp", "react", "--json"]);
    expect(result.exitCode).toBe(0);
    const parsed = JSON.parse(result.stdout);
  ...
  it("tag operations on non-existent org fail", () => {
    const result = cli(dataDir, ["admin", "org", "tag", "list", "nonexistent"]);
...
describe("CLI product tags", () => {
  ...
  beforeAll(() => {
    ({ dataDir, cleanup } = createTempDataDir());
    const s1 = cli(dataDir, ["admin", "org", "add", "Vercel"]);
    if (s1.exitCode !== 0) throw new Error(`Seed org failed: ${s1.stderr}`);
    const s2 = cli(dataDir, ["admin", "product", "add", "Next.js", "--org", "vercel"]);
  ...
  it("product tag operations on non-existent product fail", () => {
    const result = cli(dataDir, ["admin", "product", "tag", "list", "nonexistent"]);
    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain("not found");
  });
