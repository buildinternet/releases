/**
 * OAuth-JWT lane for the MCP worker (#1483). The verifier is unit-tested in
 * @releases/lib; here we prove the wiring: a verified "Sign in with Releases"
 * JWT resolves to a scoped token identity, a failure falls open to anonymous
 * (additive — the unauthenticated MCP path is never gated), and a JWT identity
 * does NOT bypass the staging access gate. The `jwtKeyResolver` opts seam avoids
 * a real JWKS fetch.
 */
...
describe("MCP OAuth-JWT lane", () => {
  ...
  it("falls open to anonymous on a wrong-audience JWT (additive)", async () => {
  ...
  it("falls open to anonymous on a tampered JWT", async () => {
    ...
    expect(r.ok && r.identity.kind).toBe("anonymous");
  });
