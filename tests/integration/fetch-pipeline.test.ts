...
describe("fetch CLI pipeline (fixture server)", () => {
  describe("feed-type source fetch", () => {
    ...
    it("fetches releases from feed source", () => {
      const result = cli(dataDir, ["admin", "source", "fetch", "test-feed", "--no-summarize"], {
        timeout: 20_000,
      });
  ...
  describe("fetch with --max flag", () => {
    ...
    it("respects --max 1 limit", () => {
      const result = cli(
        dataDir,
        ["admin", "source", "fetch", "max-test-feed", "--max", "1", "--no-summarize"],
        { timeout: 20_000 },
      );
      expect(result.exitCode).toBe(0);
  ...
  describe("fetch with --dry-run", () => {
    ...
    it("does not persist releases on --dry-run", () => {
      const result = cli(
        dataDir,
        ["admin", "source", "fetch", "dry-run-feed", "--dry-run", "--no-summarize"],
        { timeout: 20_000 },
      );
      expect(result.exitCode).toBe(0);
  ...
  describe("fetch --json output", () => {
    ...
    it("returns structured JSON result", () => {
      const result = cli(
        dataDir,
        ["admin", "source", "fetch", "json-feed", "--json", "--no-summarize"],
        { timeout: 20_000 },
      );
      expect(result.exitCode).toBe(0);
