...
describe("first fetch ignores pre-stored ETags", () => {
  ...
  beforeAll(() => {
    ...
    cli(dataDir, ["admin", "org", "add", "ETag Test Org", "--category", "cloud"]);
    cli(dataDir, [
      "admin", "source", "add", "ETag Test Feed",
  ...
  it("fetches releases even when poll has pre-stored an ETag", () => {
    ...
    const pollResult = cli(dataDir, ["admin", "source", "poll", "etag-test-feed"], { timeout: 20_000 });
    ...
    const fetchResult = cli(dataDir, ["admin", "source", "fetch", "etag-test-feed", "--no-summarize"], { timeout: 20_000 });
  ...
  it("second fetch correctly uses ETag caching", () => {
    ...
    const result = cli(dataDir, ["admin", "source", "fetch", "etag-test-feed", "--no-summarize"], { timeout: 20_000 });
