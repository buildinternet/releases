...
function parseArgs(argv: string[]): Args {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--help" || a === "-h") {
      logger.info(
        "Usage: bun scripts/export-org-manifest.ts <org-slug> [--out <dir>] [--stdout] [--api <url>]\n" +
...
async function main() {
  ...
  if (!args.slug) {
    logger.error("Missing <org-slug>. Run with --help for usage.");
    process.exit(1);
  }
  ...
  const url = `${args.api}/v1/orgs/${encodeURIComponent(args.slug)}`;
  const res = await fetch(url, { headers });
  if (!res.ok) {
    logger.error(`GET ${url} → ${res.status} ${res.statusText}`);
    process.exit(1);
  }
  const org = (await res.json()) as WireOrg;

  const manifest = buildManifest(org);
  const json = `${JSON.stringify(manifest, null, 2)}\n`;

  // Write to a temp path (or the out dir), validate through the skill's
  // bundled validator, and report. Validation runs against the same contract
  // the well-known sweep enforces, so a passing export is round-trippable.
  const outDir = args.stdout ? join(REPO_ROOT, ".context/manifests") : args.out;
  ...
  let validatorOut = "";
  try {
    validatorOut = execFileSync("node", [VALIDATOR, outPath, "--scope", "domain"], {
      encoding: "utf8",
    });
  } catch (e) {
    valid = false;
    const err = e as { stdout?: string; stderr?: string };
    validatorOut = (err.stdout ?? "") + (err.stderr ?? "");
  }
  ...
  logger.info(
    `${valid ? "VALID" : "INVALID"}  ${org.slug}  ` +
      `(${productCount} products, ${locatorCount} locators)  → ${outPath}`,
  );
  if (validatorOut.trim()) logger.info(validatorOut.trim());
