 ...
 *
 * We use this to bound short-lived local D1 one-shots (`db:migrate:local`,
 * `db:query`, and the migrate step inside `db:reset:local`). A coding agent that
 * times out mid-command can otherwise leave a `wrangler d1 … --local` miniflare
 * boot running detached, where it hangs and grows to multiple GB of RSS. The
 * long-lived `wrangler dev` / `preview:*` servers are intentionally NOT wrapped —
 * those are torn down by the session/worktree scripts.
 *
 * Usage:
 *   node scripts/run-timed.mjs <seconds> -- <cmd> [args…]
 *
 * Library:
 *   import { runTimed } from "./run-timed.mjs";
 *
