...
- **User follows + feed**: signed-in users follow orgs/products (`user_follows`, worker-local schema island); session-authed `/v1/me/follows` + `/v1/me/feed` (org follow = its products too), enabled by default (no feature flag — the cookie session is the only gate). See [routing.md](docs/architecture/routing.md) and [web.md](docs/architecture/web.md).
