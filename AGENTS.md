...
- **Source staleness signal + render dry-run (#1528):** a daily warn-only scan (`cron/source-staleness.ts`, `source-staleness` component, rides the `0 4 * * *` tick) flags established-cadence (`medianGapDays != null`), still-polled, non-Firecrawl sources whose newest release is past `max(SOURCE_STALE_FLOOR_DAYS=14, medianGapDays × SOURCE_STALE_MULTIPLIER=3)` — first-party staleness so Firecrawl is a hedge, not a requirement. `POST /v1/sources/:id/fetch?dryRun=true` on a client-rendered scrape source (`crawlEnabled`/`renderRequired`) renders the index once and returns `{ renderCheck, candidateCount, sampleUrls }` with no extraction/MA loop (CLI: `releases source fetch <src> --dry-run`). No feature flag (observe-only). See [firecrawl-monitoring.md](docs/architecture/firecrawl-monitoring.md).
...
- **Local ingest** (local Claude Code, no remote MA): the `local-ingest` skill (`src/agent/skills/local-ingest/`) has the agent fetch + extract releases itself and write via the idempotent `/batch` upsert — skipping the coordinator-Sonnet/worker-Haiku extraction loop and its inference bill. Mandatory `robots.txt`/`Content-Signal` opt-out preflight (`preflight.ts`) refuses `ai-input=no`/`ai-train=no` (e.g. `conductor.build`). See [local-ingest.md](docs/architecture/local-ingest.md).
...
- Workflows-based ingest: the daily scrape-agent sweep (#482) and hourly poll-and-fetch (#486) run as Cloudflare Workflows whenever their Workflow binding is wired (always in prod); per-source tier intervals (normal=4h, low=24h) gate what's due; start is smeared across `FANOUT_JITTER_WINDOW_MS`. The inline crons stay as the binding-absent fallback. See [remote-mode.md](docs/architecture/remote-mode.md).
...
- **Owner-declared listing metadata**: a `$schema`-validated `releases.json` — domain `.well-known/releases.json` sets org identity, repo-root `releases.json` maps that source to a product (grouping by shared product slug); display-only, fail-closed, never clobbers curator/editorial fields. See [well-known-config.md](docs/architecture/well-known-config.md).
- **User follows + feed**: signed-in users follow orgs/products (`user_follows`, worker-local schema island); `/v1/me/follows` + `/v1/me/feed` (org follow = its products too), enabled by default (no feature flag). The gate accepts a Better Auth session OR a Bearer **user** principal (`relu_` key / OAuth JWT) so the CLI + MCP can manage follows too (NOT relk\_/root/anonymous — these have no owning user); MCP exposes `follow`/`unfollow`/`list_follows`/`get_personalized_feed`, forwarding the caller's credential to `/v1/me/*`. See [routing.md](docs/architecture/routing.md), [web.md](docs/architecture/web.md), and [mcp.md](docs/architecture/mcp.md).
