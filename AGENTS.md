...
**Carved-out packages:** Shared code is being progressively moved out of `src/` into `packages/`:
- `packages/core/` (`@releases/core/*`) — pure helpers with no Bun-isms: `schema`, `release-upsert`, `categories`, `dates`, `tokens`, `changelog-slice`, `changelog-range`, `id`, `slug`, `hash`.
...
- `packages/lib/` (`@releases/lib/{config,errors,logger}`) — runtime-neutral CLI/runtime helpers. The Bun-specific pieces of the old `src/lib/config.ts` were split out to `src/agent/cli-cmd.ts`.

Many files still live in `src/lib/` (media, media-url, formatters, embed-*, vector-search, search-hybrid, embeddings, mode, discover, telemetry, providers, etc.) and have not moved yet. Workers (`workers/{api,mcp,discovery}`) use tsconfig path mappings where `@releases/lib/*` falls back to `../../src/lib/*` for files that haven't been carved out.

## Conventions

- All logging goes to **stderr** (`@releases/lib/logger`, source at `packages/lib/src/logger.ts`). stdout is reserved for MCP JSON-RPC in serve mode.
...
- `daysAgoIso()` from `@releases/core/dates` (source at `packages/core/src/dates.ts`) for date cutoff calculations.
- CLI commands that return data support `--json` for machine-readable output.
- Batch DB inserts in chunks of 500 (SQLite variable limit). In remote mode, `insertReleasesBatch` in `src/api/client.ts` posts 100 releases per HTTP request to `POST /v1/sources/:slug/releases/batch` — D1's practical cap for a single INSERT statement is ~100 rows before the query size approaches its 1MB limit. The worker's inner loop also chunks at 100 defensively.
- Dedup via `UNIQUE(source_id, url)` and the shared `RELEASE_URL_UPSERT` config in `@releases/core/release-upsert` (source at `packages/core/src/release-upsert.ts`) — on URL collision, content is backfilled when the incoming row is non-empty and the existing row is empty. Both the local (`src/db/queries.ts`) and worker (`workers/api/src/routes/sources.ts`) batch-insert paths import that helper so they can't drift.
...
**Release flow (Changesets):** every PR that changes user-visible behavior (CLI commands, MCP tools, REST routes, deployment requirements) must ship with a `bun run changeset` entry — commit the generated `.changeset/*.md` file alongside your code changes.
...
2. **On merge to main**: the `Release CLI` workflow (`.github/workflows/release.yml`) runs `changesets/action@v1`. If there are pending changesets, it opens (or updates) a `chore: version packages` PR that runs `bun run changeset:version` — that calls `changeset version` to bump `npm/*/package.json` + write `CHANGELOG.md`, then `scripts/sync-version.ts` mirrors the new version back to root `package.json`, `src/cli/program.ts`, and `src/mcp/server.ts`, then `bun install` refreshes the lockfile.
3. **Merging the version PR** triggers the workflow again, changesets/action detects there are no pending changesets and runs `bun run changeset:publish` to publish all five packages to npm. Subsequent steps gzip the binaries, publish them as a GitHub Release on the public mirror (`buildinternet/releases`, tagged `v<version>`), and regenerate `Formula/releases.rb` in the public tap (`buildinternet/homebrew-tap`) with the new version + SHAs. Both cross-repo writes use the `TAP_PUBLISH_TOKEN` secret (a fine-grained PAT with write access to both).

**Homebrew distribution:** the source of truth for the formula is the public tap repo — the monorepo does not carry a copy. Hand-edits to the formula (e.g., `depends_on`, `livecheck`) should be made in the tap heredoc inside `.github/workflows/release.yml`, since CI overwrites the tap copy on every publish. The binaries it points at live as GitHub Release assets on `buildinternet/releases` (a public binaries-only mirror — no source), which keeps this monorepo private while still letting Homebrew do anonymous downloads.
...
The API Worker lives at `workers/api/` and shares the Drizzle schema from `@releases/core/schema` (`packages/core/src/schema.ts`). D1 migrations are in `workers/api/migrations/`. Deploy with `cd workers/api && wrangler deploy`.
