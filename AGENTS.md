...
- **User follows + feed**: signed-in users follow orgs/products (`user_follows`, worker-local schema island); session-authed `/v1/me/follows` + `/v1/me/feed` (org follow = its products too), gated by `user-follows-enabled`. See [routing.md](docs/architecture/routing.md) and [web.md](docs/architecture/web.md).
