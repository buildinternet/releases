...
- Logging splits by runtime:
  - **CLI + runtime-neutral packages** (`packages/adapters/`, `packages/ai/`, `packages/lib/`, `scripts/`, `tests/evals/`, `src/agent/`) log via `@buildinternet/releases-lib/logger` (source `packages/lib/src/logger.ts`) — writes to stderr **and** persists per-day files under `~/.releases/logs/` (Node/Bun only; that's the point of it).
  - **Worker code** (`workers/api/`, `workers/mcp/`, `workers/discovery/`, `workers/webhooks/`) emits structured JSON via `logEvent()` from `@releases/lib/log-event` (worker-safe, no `fs`). Carry `component` + kebab-case `event` as top-level keys (Workers Logs indexes them as filter fields); severity comes from which `console.*` the helper calls, so **don't** put `level` in the payload; it unwraps `Error` to `{name,message,stack,cause?}`. New worker code MUST use `logEvent`; existing plain `console.*` migrates per-touch. **Never** introduce the `@buildinternet/releases-lib/logger` into a worker — its `fs` is a per-request virtual discard and it double-tags `[releases]`.
