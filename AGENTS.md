...
- Pagination shape: catalog-shaped surfaces (stable, sortable, mostly non-mutating between calls — the four MCP `list_*` tools, `/v1/sources`, `/v1/orgs`, `/v1/products`) use **page-based** pagination (`page` + `limit` inputs, `Pagination { page, pageSize, returned, totalItems, totalPages, hasMore }` output). Feed-shaped surfaces (append-only, mutates between calls — `/v1/orgs/:slug/releases`, `/v1/status/fetch-log`, future `get_latest_releases` continuation) and ranking-bounded surfaces (`search`, `search_releases`, `search_registry`) use **cursor-based** pagination (opaque `cursor` input, `nextCursor` output). Page-based is the default when a surface is bounded and stable; pick cursor only when the data shape forces it. MCP `list_*` tool results expose pagination via `_meta.pagination` (matching the REST `Pagination` shape) plus an LLM-readable markdown footer.
