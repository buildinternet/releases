...
- Workflows-based ingest: the scrape-agent sweep (live, issue #482) and the 2-hourly poll-and-fetch (flag-off, issue #486) both have Cloudflare Workflows paths gated by `SCRAPE_AGENT_USE_WORKFLOW` / `POLL_FETCH_USE_WORKFLOW`. When on, each cron phase becomes a `step.do` boundary so embed retries are independent of fetch retries. Inline crons are the rollback.
