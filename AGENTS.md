...
- `packages/adapters/` (`@releases/adapters/*`) — adapter-shared primitives (batch 1): `types`, `source-meta`, `content-hash`. The remaining adapters (`github`, `agent`, `scrape`, `feed`, `cloudflare`, `crawl`) still live in `src/adapters/` and will be carved out in a future PR.
