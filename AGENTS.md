...
- `packages/lib/` — `@releases/lib/{config,errors,atom,atom-http,embeddings,embedding-cache,vector-search,embed-changelogs,embed-changelog-pipeline,embed-entities,embed-releases}` private. `logger` is published as `@buildinternet/releases-lib/logger`.

Worker tsconfigs map `@releases/lib/*` first to `packages/lib/src/*` (published carve-outs) and fall through to `src/lib/*` for files not yet carved out (formatters, media helpers, source-edit).

## Surviving `src/` tree

- `src/api/types.ts` — shared API types, imported by `web/` and all workers (pinned in worker tsconfig `files[]`).
- `src/lib/` — web + worker shared helpers still in flight for carve-out: `formatters`, `media`, `media-url`, `source-edit`.
