...
3. **Merging the version PR** triggers the workflow again, changesets/action detects there are no pending changesets and runs `bun run changeset:publish` to publish all five packages to npm. Subsequent steps gzip the binaries, publish them as a GitHub Release on the public mirror (`buildinternet/releases-cli`, tagged `v<version>`), and regenerate `Formula/releases.rb` in the public tap (`buildinternet/homebrew-tap`) with the new version + SHAs. Both cross-repo writes use the `TAP_PUBLISH_TOKEN` secret (a fine-grained PAT with write access to both).

**Homebrew distribution:** the source of truth for the formula is the public tap repo — the monorepo does not carry a copy. Hand-edits to the formula (e.g., `depends_on`, `livecheck`) should be made in the tap heredoc inside `.github/workflows/release.yml`, since CI overwrites the tap copy on every publish. The binaries it points at live as GitHub Release assets on `buildinternet/releases-cli` (a public binaries-only mirror — no source), which keeps this monorepo private while still letting Homebrew do anonymous downloads. The name `buildinternet/releases` is reserved for a future public move of this monorepo.
