...
- Route naming (issue #494): resource CRUD lives on the canonical path (`/v1/<resource>/...`) with auth gated by the `adminRoutes` allowlist in `workers/api/src/index.ts`. Job / side-effect triggers (summarize, compare, embed backfills, notifications-test) live under `/v1/workflows/<job-name>` in `workers/api/src/routes/workflows.ts`. Legitimate admin-only telemetry reads that don't fit either bucket (cron-runs list/detail, embed/status) stay under `/v1/admin/...`. Do not add new `/v1/admin/*` endpoints for CRUD or for async triggers.
