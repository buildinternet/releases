...
- `parseCoordinate()` from `@buildinternet/releases-core/lookup-coordinate` parses a bare `"org/repo"` string into `{ provider: "github", org, repo }` (returns `null` on miss). The `GITHUB_SEGMENT` regex (`/^[A-Za-z0-9._-]+$/`) rejects colons, so the `"provider:org/repo"` form is intentionally **not** accepted today — adding it requires extending the parser and updating callers (`workers/api/src/routes/lookups.ts`, `workers/api/src/routes/search.ts`, `workers/mcp/src/mcp-agent.ts`).
