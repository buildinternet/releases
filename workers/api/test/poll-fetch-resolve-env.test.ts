...
describe("resolveFetchEnv (poll-and-fetch workflow)", () => {
  ...
  // the AI-only guard never covered. (That occurrence was `INDEXNOW_KEY`, whose
  // feature has since been removed in #2201; the guard it forced stays.)
  ...
  // Used to build the ping target. Falls back to `https://releases.sh`, so a
  // drop is invisible in prod and wrong everywhere else — staging/dev would
  // silently aim at the production host.
  it("forwards WEB_BASE_URL so the ping doesn't fall back to the prod host", async () => {
    const fetchEnv = await resolveFetchEnv(buildEnv());
