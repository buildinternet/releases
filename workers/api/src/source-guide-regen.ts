/**
 * Auto-regenerate the source guide header after source mutations.
 *
...
import { generateSourceGuideHeader, extractNotesFromLegacyGuide } from "@releases/ai/source-guide.js";
import { newKnowledgePageId } from "./utils.js";

/**
 * Regenerate the source guide header for an org. Fire-and-forget safe — catches all errors.
 *
 * If the org has an old-format guide (header + notes combined), this migrates it:
 * notes are extracted into the notes column and the content is replaced with header-only.
 */
export async function regenerateSourceGuide(
  ...
  try {
    ...
    // Check for existing guide — migrate notes from old format if needed
    ...
    const notes: string | null = existing?.notes
      ?? (existing ? extractNotesFromLegacyGuide(existing.content) : null);
    ...
    await db.run(sql`INSERT INTO knowledge_pages (id, scope, org_id, product_id, content, notes, release_count, last_contributing_release_at, generated_at, updated_at)
      VALUES (${id}, 'source-guide', ${orgId}, NULL, ${header}, ${notes}, ${orgSources.length}, NULL, ${now}, ${now})
    ...
    // Fire-and-forget — don't let guide regen failures break source mutations
  }
}
