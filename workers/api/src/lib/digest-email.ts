...
export function buildDigestEmail(content: DigestEmailContent): {
  ...
  const orgNames = groups.map((g) => g.orgName);
  ...
  const blocks: EmailBlock[] = groups.map((g) => {
    const { posts, rollups } = partitionOrgItems(g.items);
    return {
      t: "orgGroup",
      name: g.orgName,
      url: g.orgSlug ? `${baseUrl}/${g.orgSlug}` : undefined,
      avatarUrl: g.avatar,
      posts: posts.map((r) => ({
        title: bestTitle(r),
        url: releaseUrl(baseUrl, r),
        summary: postSummary(r),
        meta: r.product ? r.product.name : null,
        highSignal: isHighSignal(r),
        // "Bug fixes and small improvements" shouldn't read as loudly as a real
        // feature post, so it stays ink rather than becoming an accent link.
        muted: isLowSignalPost(r),
      })),
      // Per-product rollup: one row — product, the representative version, and
      // "and N more". Deliberately not an enumerated version list.
      rollups: groupByProduct(rollups).map((tg) => {
        const { rep, blurb, hiddenCount } = rollupView(tg.items);
        return {
          product: tg.label,
          count: tg.items.length,
          version: versionLabel(rep),
          blurb: blurb ? `Latest — ${blurb}` : null,
          more: hiddenCount > 0 ? `and ${hiddenCount} more` : null,
          url: hiddenCount > 0 ? productPageUrl(baseUrl, rep) : releaseUrl(baseUrl, rep),
          versionUrl: releaseUrl(baseUrl, rep),
        };
      }),
    };
  });
  ...
  const { html, text } = renderEmail({
    ...
    // The inbox button opens the feed the digest was built from.
    action: { kind: "view", name: "View feed", url: manageUrl },
  });
