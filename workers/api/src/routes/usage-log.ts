...
import { usageLog } from "@releases/db/schema.js";
...
usageLogRoutes.get("/usage-log/stats", async (c) => {
  ...
  const [[totals], byOperation, byModel, bySource] = await Promise.all([
    ...
    }).from(usageLog).where(gte(usageLog.createdAt, since)),
    ...
    db.select({
      ...
      totalOutput: sql<number>`COALESCE(SUM(${usageLog.outputTokens}), 0)`,
      count: sql<number>`COUNT(*)`,
    }).from(usageLog).where(gte(usageLog.createdAt, since)).groupBy(usageLog.operation),
    ...
    db.select({
      ...
      totalOutput: sql<number>`COALESCE(SUM(${usageLog.outputTokens}), 0)`,
      count: sql<number>`COUNT(*)`,
    }).from(usageLog).where(gte(usageLog.createdAt, since)).groupBy(usageLog.model),
    ...
    db.select({
      ...
      totalOutput: sql<number>`COALESCE(SUM(${usageLog.outputTokens}), 0)`,
      count: sql<number>`COUNT(*)`,
    }).from(usageLog).where(and(gte(usageLog.createdAt, since), isNotNull(usageLog.sourceSlug))).groupBy(usageLog.sourceSlug),
  ]);
