...
import { fetchOverviewCitations } from "../lib/overview-citations.js";
...
app.get(
  ...
  describeRoute({
    ...
    description:
      "Returns the AI-generated knowledge page for the org, including its source citations (each with a canonical internal `releaseWebUrl` when the source resolved to an on-registry release). Returns `null` when no overview has been generated yet. The org is resolved by slug or typed `org_…` ID.",
    responses: {
      200: {
  ...
  async (c) => {
    ...
    const citationRows = await fetchOverviewCitations(db, row.id, c.env);
...
app.post(
  ...
  describeRoute({
    ...
    description:
      "Creates or replaces the org's AI-generated knowledge page. Accepts the markdown `content`, the `releaseCount` it was derived from, the ISO timestamp of the most-recent contributing release (`lastContributingReleaseAt`), and an optional `citations` array of source objects (`sourceUrl` + optional `title`). Citations are replace-all — omitting the field clears any existing citations. Requires Bearer auth inherited from `publicReadAuthMiddleware`'s non-SAFE_METHODS branch.",
    security: [{ bearerAuth: [] }],
    responses: {
      200: {
  ...
  async (c) => {
    ...
    const result = await upsertOrgOverview(db, {
      ...
      // Citations are a source list now (#1934); any legacy offset/quote fields
      // on the incoming row are accepted-but-ignored.
