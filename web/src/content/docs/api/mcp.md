...
> **Identifiers:** every tool that takes an org / product / source / release identifier accepts the typed ID (`org_…`, `prod_…`, `src_…`, `rel_…`) interchangeably with the slug. Source and product params also accept an `org/slug` coordinate (e.g. `vercel/nextjs`). Each tool's `inputSchema.description` lists the concrete shapes it accepts.
