const REMOTE_URL = "https://mcp.releases.sh/mcp";

const stdioConfig = {
  command: "npx",
  args: ["mcp-remote", REMOTE_URL],
};

const cursorConfigB64 = Buffer.from(JSON.stringify(stdioConfig)).toString("base64");
const cursorHref = `cursor://anysphere.cursor-deeplink/mcp/install?name=releases&config=${cursorConfigB64}`;

const vscodePayload = encodeURIComponent(
  JSON.stringify({ name: "releases", ...stdioConfig }),
);
const vscodeHref = `vscode:mcp/install?${vscodePayload}`;
const vscodeInsidersHref = `vscode-insiders:mcp/install?${vscodePayload}`;

type Button = {
  label: string;
  href: string;
  icon: React.ReactNode;
};

const buttons: Button[] = [
  {
    label: "Add to Cursor",
    href: cursorHref,
    icon: (
      <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        <path d="M11.925 24l10.425-6-10.425-6L1.5 18l10.425 6z" opacity=".7" />
        <path d="M22.35 18V6L11.925 0v12l10.425 6z" opacity=".9" />
        <path d="M11.925 0L1.5 6v12l10.425-6V0z" />
      </svg>
    ),
  },
  {
    label: "Add to VS Code",
    href: vscodeHref,
    icon: (
      <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        <path d="M23.15 2.587L18.21.21a1.494 1.494 0 0 0-1.705.29l-9.46 8.63-4.12-3.128a.999.999 0 0 0-1.276.057L.327 7.261A1 1 0 0 0 .326 8.74L3.899 12 .326 15.26a1 1 0 0 0 .001 1.479L1.65 17.94a.999.999 0 0 0 1.276.057l4.12-3.128 9.46 8.63a1.492 1.492 0 0 0 1.704.29l4.942-2.377A1.5 1.5 0 0 0 24 20.06V3.939a1.5 1.5 0 0 0-.85-1.352zm-5.146 14.861L10.826 12l7.178-5.448v10.896z" />
      </svg>
    ),
  },
  {
    label: "VS Code Insiders",
    href: vscodeInsidersHref,
    icon: (
      <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        <path d="M23.15 2.587L18.21.21a1.494 1.494 0 0 0-1.705.29l-9.46 8.63-4.12-3.128a.999.999 0 0 0-1.276.057L.327 7.261A1 1 0 0 0 .326 8.74L3.899 12 .326 15.26a1 1 0 0 0 .001 1.479L1.65 17.94a.999.999 0 0 0 1.276.057l4.12-3.128 9.46 8.63a1.492 1.492 0 0 0 1.704.29l4.942-2.377A1.5 1.5 0 0 0 24 20.06V3.939a1.5 1.5 0 0 0-.85-1.352zm-5.146 14.861L10.826 12l7.178-5.448v10.896z" />
      </svg>
    ),
  },
];

export function McpInstallButtons() {
  return (
    <div className="not-prose my-6 flex flex-wrap gap-2">
      {buttons.map((b) => (
        <a
          key={b.label}
          href={b.href}
          className="inline-flex items-center gap-2 rounded-md border border-stone-200 bg-white px-3 py-2 text-[13px] font-medium text-stone-800 shadow-sm transition-colors hover:border-stone-300 hover:bg-stone-50 dark:border-stone-700 dark:bg-stone-900 dark:text-stone-100 dark:hover:border-stone-600 dark:hover:bg-stone-800"
        >
          <span className="text-stone-600 dark:text-stone-300">{b.icon}</span>
          {b.label}
        </a>
      ))}
    </div>
  );
}
