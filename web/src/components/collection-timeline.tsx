...
function CollectionFiltersMenu({
  ...
  return (
    <FiltersPopover
      ...
      triggerClassName="h-7 shrink-0 rounded-md border border-stone-200 bg-white px-2 dark:border-stone-700 dark:bg-stone-900"
    >
      <FilterMenuSection label="Type">
        <div role="group" aria-label="Release type">
          {TYPE_FILTERS.map((opt) => (
            <FilterMenuRadio
              key={opt.id}
              checked={typeFilter === opt.id}
              onSelect={() => onTypeFilterChange(opt.id)}
            >
              {opt.label}
            </FilterMenuRadio>
          ))}
