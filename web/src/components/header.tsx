...
export function Header() {
  return (
    <header className="border-b border-stone-200 dark:border-stone-800 px-6 py-4 flex items-center justify-between">
      <SearchHotkey />
      <Link href="/" className="font-bold text-base tracking-tight text-stone-900 dark:text-stone-100 flex items-center gap-2">
        <svg viewBox="0 0 64 64" aria-hidden="true" className="h-5 w-5 shrink-0">
          <rect width="64" height="64" rx="14" fill="currentColor" className="text-stone-900 dark:text-stone-100" />
          <rect x="14" y="18" width="28" height="6" rx="1.5" className="fill-stone-50 dark:fill-stone-900" />
          <rect x="14" y="29" width="22" height="6" rx="1.5" className="fill-stone-50/70 dark:fill-stone-900/70" />
          <rect x="14" y="40" width="36" height="6" rx="1.5" fill="oklch(0.60 0.18 252)" />
        </svg>
        releases.sh<span className="text-[10px] font-medium uppercase tracking-wider text-stone-400 dark:text-stone-500 border border-stone-300 dark:border-stone-700 rounded px-1.5 py-0.5 leading-none">preview</span>
      </Link>
