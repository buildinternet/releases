      ...
      <div className="border border-amber-200 bg-amber-50 rounded-lg p-6">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h2 className="font-semibold text-stone-900 text-sm mb-1">Setup required</h2>
            <p className="text-sm text-stone-600 mb-4">{message}</p>
            <pre className="bg-stone-900 text-stone-100 text-xs rounded-md p-4 overflow-x-auto whitespace-pre-wrap break-words">
              {steps.map((step, i) => (
                <span key={i}>
                  {step.startsWith("#") ? (
