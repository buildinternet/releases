...
function post(body: unknown = { message: "wrong release date on this listing", type: "bug" }) {
  return new NextRequest("https://releases.sh/api/feedback", {
    method: "POST",
    headers: { "content-type": "application/json" },
...
describe("POST /api/feedback proxy", () => {
  it("flattens the worker's nested error envelope to { error: <code> }", async () => {
    stubUpstream(400, {
      error: { code: "bad_request", type: "validation", message: "Message too short." },
    });
  ...
  it("passes a successful response through verbatim", async () => {
    stubUpstream(202, { ok: true, id: "fb_abc123" });
    const res = await POST(post());
    expect(res.status).toBe(202);
    expect(await res.json()).toEqual({ ok: true, id: "fb_abc123" });
  });
  ...
  it("400s an unparseable request body without calling upstream", async () => {
    ...
    const req = new NextRequest("https://releases.sh/api/feedback", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: "{not json",
    });
