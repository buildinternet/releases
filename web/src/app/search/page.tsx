  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-2xl mx-auto px-6 pt-12 pb-12">
        <h1 className="text-[28px] font-bold tracking-tight text-stone-900 mb-6 text-center">Search</h1>
                    ...
                    <div key={i} className="border-b border-stone-200 py-4 first:pt-0 last:border-b-0">
                      <div className="flex justify-between items-baseline mb-1">
                        <div className="flex items-baseline gap-2">
                          {r.version && <span className="font-semibold text-[15px] text-stone-900">{r.version}</span>}
                          <span className="text-sm text-stone-600">{r.title}</span>
                        </div>
                        <span className="text-xs text-stone-400 whitespace-nowrap ml-4">{formatDate(r.publishedAt)}</span>
                      </div>
                      <p className="text-[13px] text-stone-500 mb-1">{r.summary}</p>
                      <Link href={href} className="text-xs text-stone-400 hover:text-stone-600">{r.sourceName}</Link>
                    </div>
                  );
                })}
              </div>
